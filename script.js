...
function initMusicPlayer() {
    ...
    console.log('- 音乐切换按钮:', musicToggleBtn);
    ...
    // 显示音乐功能就绪信息 - 强调用户交互
    const readyMessage = '🎵 背景音乐功能已就绪，请点击音符图标开始播放 🎵';
...
function toggleMusic() {
    ...
    isMuted = originalIsMuted; // 恢复原始静音状态
}
...
function pauseMusic() {
    if (isMusicPlaying && audioElement) {
        audioElement.pause();
        isMusicPlaying = false;
        showFeedback('', 'info', false); // 不启用语音提示
        console.log('', audioElement.currentTime, '秒');
    }
}
